...
function notifyBroadcasted(){
    userWithdrawRequest.find({is_exchange_notified: false},function(error,result){
        if(error){
            console.log(error);
            ...
            for(let tx of result){
                ...
                exchangeAPI.notifyExchange(globalConfig.exchangeURL+'user/withdrawSuccess', params).then(result =>{
                    console.log(result);
                    userWithdrawRequest.findOneAndUpdate({transaction_hash:tx.transaction_hash},{is_exchange_notified:true, exchange_status:tx.block_confirmations},function(error,result){
                        console.log("Exchange notified and logged in database for tx id : "+ tx.transaction_id);
                    });
                }).catch(e=>{
                    console.log(e);
                });
